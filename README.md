...
unmodified counterparts but make the Assist Menu *sticky*: after use,
the menu remains open to use again.
...
Our configuration also defines <kbd>,</kbd> and <kbd>.</kbd> as the
...
`:key-am-mod-m-qk nil`. And while not necessary to avoid a conflict, for
clarity and consistency we also set `:key-am-mod-c-qk nil`.

> Note that, by default, the Assist Menu also reserves <kbd>A</kbd>
> <kbd>C</kbd> <kbd>H</kbd> <kbd>M</kbd> and <kbd>S</kbd> as the
> [sticky](#sticky-keys) versions of <kbd>a</kbd> <kbd>c</kbd>
...
> there does remain a means to toggle <kbd>Control</kbd> and
> <kbd>Meta</kbd> from the Assist Menu if desired.
...
1.  <kbd>, m C ,</kbd> or <kbd>. m M .</kbd>
    ...
    This method uses the Assist Menu (once), but uses a [sticky
    key](#sticky-keys) to keep the menu open.
...
Without this rule, <kbd>, ,</kbd> would be translated to <kbd>C-,</kbd>.
